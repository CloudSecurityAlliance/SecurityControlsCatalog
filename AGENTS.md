...
Catalog content is committed under `objects/<stix-type>/<secid-path>.json` — currently AICM 1.1.0 controls, generated by `tools/generate_aicm_controls.py`. Inspect the tree (`git ls-files`) before adding structure, and keep layout proposals explicit in the pull request. Generated content is committed rather than built at publish time, because the identifiers must be stable; never re-mint an identifier an existing file already holds.
