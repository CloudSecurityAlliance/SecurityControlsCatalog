...
For future structured data, prefer standard STIX 2.1 fields and relationships before adding custom `x-*` properties. **New object types are declared with STIX 2.1's `extension-definition` mechanism, never as bare `x-`-prefixed custom objects** — one definition per type, referenced from each instance via `extensions` with `extension_type: "new-sdo"`; the definitions travel with the content in every published bundle and their identifiers are permanent once published. **Express relationships as standard `relationship` SROs, never as arrays of identifiers inside an object** — a mapping is a claim needing rationale, confidence, and authorship, and generic STIX tooling can traverse `source_ref`/`target_ref` but not a custom property. The exception is a reference constitutive of its object (`x-control-assessment.assessed_control_ref`, mirroring `sighting.sighting_of_ref`): if removing the reference makes the object incoherent it is a property, otherwise it is a relationship. Objects carry a [SecID](https://secid.cloudsecurityalliance.org/) (for example `secid:control/cloudsecurityalliance.org/ccm@4.1#CEK-03`) so they resolve through SecID's public resolver, but a SecID does **not** go in the STIX `id` property — STIX 2.1 requires `id` to be `<type>--<UUIDv4>`. Carry it in `external_references`, decompose its components into queryable properties for third-party content, and do not invent a third identifier scheme. Keep filenames uppercase only for top-level policy or contributor documents such as `LICENSE.txt` and `AGENTS.md`; use descriptive kebab-case for new design notes unless the repository establishes another pattern.
...
- **Rejected as back-end formats:** CSAF 2.0, OSV 1.3, CVE JSON 5.2.0, and CVRF 1.2 (advisories and vulnerability records, not control catalogs), plus RDF, JSON-LD, and OWL (duplicate-purpose given SecID identity and STIX SROs).
