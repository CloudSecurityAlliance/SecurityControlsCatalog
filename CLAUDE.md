...
Each document governs its own column: the rationale settles format and publication, the conventions settle modeling idioms, the schema settles fields. Where two documents touch the same subject, the one whose column it is governs — not the newer one. Never reproduce field definitions from memory or infer them from other STIX extensions — read the schema document. **Both the conventions and schema documents carry an `Open questions` section listing what is genuinely undecided** (data markings, `status` vs. `revoked`, SecID placement, vocabularies, cardinality, `confidence` scales). Raise an issue rather than inventing an answer to any of them in instance data.
