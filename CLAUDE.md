...
Objects carry a [SecID](https://github.com/CloudSecurityAlliance/SecID) (for example `secid:control/cloudsecurityalliance.org/scc/...`) so they resolve through SecID's public resolver. A SecID does **not** go in the STIX `id` property: STIX 2.1 requires `id` to be `<type>--<UUIDv4>`, so a `secid:` URI there is invalid STIX and breaks the maximum-compatibility principle. Carry it in `external_references` or a dedicated property. Do not invent a third identifier scheme.
