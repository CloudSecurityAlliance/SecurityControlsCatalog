...
- **Maximum STIX compatibility.** No changes to STIX wire format, versioning, or transport. Custom SDOs use standard STIX properties (`id`, `spec_version`, `created`, `modified`, `created_by_ref`, `labels`, `external_references`, etc.) and participate in relationships exactly like any other SDO.
- **New types are declared with `extension-definition`, not bare custom objects.** The `x-`-prefixed-type-plus-convention approach is a STIX 2.0 practice that 2.1 discourages; a new SDO gets an `extension-definition` object and every instance references it via `extensions` with `extension_type: "new-sdo"`. One definition per type. The definitions must ship in every published bundle, and their identifiers are permanent once published — consumers key off them.
...
- **Relationships are SROs, never embedded reference arrays.** A mapping, an implementation fulfilling a control, a control mitigating a threat — each is a standard `relationship` object, because each is a *claim* needing rationale, confidence, and authorship, and because generic STIX tooling can traverse `source_ref`/`target_ref` and cannot traverse a custom property. The exception is a reference that is constitutive of its object (`x-control-assessment.assessed_control_ref`, mirroring `sighting.sighting_of_ref`). Test: if the reference can be added, revised, or withdrawn while the object stays meaningful, it is a relationship; if removing it makes the object incoherent, it is a property.
...
- **Compatibility with SecID.** [SecID](https://secid.cloudsecurityalliance.org/) and this catalog are **co-evolving projects working the same problem space** — security controls, regulations, mappings, and the identifiers that make them referenceable. Neither is subordinate to the other. The catalog shares SecID's type vocabulary and its structure for third-party provenance, and uses its dataset metadata for source licensing, because solving the same problems twice in incompatible ways helps nobody. Prefer staying compatible over inventing a parallel scheme. **SecID's registry is the current source for its own vocabulary** — consult the resolver or its MCP server rather than treating a table here as up to date.
- **Divergence is expected, and can resolve in either direction.** `x-control-implementation` and `x-control-assessment` have no SecID counterpart today; SecID in turn covers ground the catalog does not. Those are places the two projects can learn from each other, not deficits the catalog owes SecID. Where closer alignment is worth having, it is a deliberate change carrying a migration path for published content — never an automatic rename, and never a pre-emptive one made in anticipation of what the other project might do.
