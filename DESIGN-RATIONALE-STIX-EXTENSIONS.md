...
- **Default to current versions, gated on adoption.** Where the catalog depends on or references something versioned — a specification, a control framework, a questionnaire — target the current version. The exception is a version that has not yet been taken up in practice: an unimplemented standard is not yet interoperable, and interoperability is the point. STIX 2.1 is the present example; a later STIX version would be adopted when tooling follows it, not on the day it publishes. This is a rule about **what new content targets, not about dropping support** — the catalog carries multiple framework versions side by side through source-provenance properties, and consumers on an earlier version such as CCM 4.0 keep a first-class experience.
